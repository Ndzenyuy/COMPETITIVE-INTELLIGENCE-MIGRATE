...
# ─── Private Subnets (RDS and EFS live here) ─────────────────────────────────
# No public IPs, no internet route. ECS tasks run in public subnets instead.
...
# Private route table: local VPC traffic only (RDS and EFS — no internet needed)
